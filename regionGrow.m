...
%大于reg_maxdis,区域生长结束
max_neg_list = cmpGate;


while (max_neg_list>=cmpGate)% && reg_size < numel(I))%&& pixdist < 1.5 max_neg_list>=cmpGate
 %增加新的邻域像素到neg_list中
 for j=1:4
     xn = x + neigb(j,1);
     yn = y + neigb(j,2);
...
neg_pos = neg_pos -1;
max_neg_list = max(neg_list(1:neg_pos,3));
end
...
% global YYs
% figure
% surf(XXs,YYs,J.*I+~J*cmpGate); %db(Ex0)
% shading flat;
% view(2);
% axis tight;
% xlabel('X (m)');
